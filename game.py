...
class Grid:
    def __init__(self, ui=False):
        self.width = 25
        self.height = 25
        # dictionary of nodes -> keys:tuple[int i,int j] : values:Node Class
        ...
        self.reset_full() # reset all colors of each node in the dictionary
        self.random() # reset goal / start nodes and reset each puddle in the grid with randomizer

    # save the test case as buffer string format in the filename
    ...
    # load the buffer test case in the filename
    ...
    # reset all the nodes in the path in the dictionary coords.
    def reset(self):
        for coord in self.nodes:
            self.nodes[coord].reset()

    # reset everythiing of each nodes in the dictionary coords.
    def reset_full(self):
        for coord in self.nodes:
    ...
    # update what's happening on the pygame screen
    ...
    # i feel like this is the same as reset()
    def clear_path(self):
        for node in self.nodes.values():
            node.reset()
